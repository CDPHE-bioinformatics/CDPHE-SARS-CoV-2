...
task hostile {
  ...
  String extension = if sub(basename(fastq1), "\.fastq$", "") == basename(fastq1, ".fastq") then ".fastq" else ".fastq.gz"
  ...
  command <<<
    ...
    if [[ "~{seq_method}" == "OXFORD_NANOPORE" ]]; then
      ...
      hostile clean \
        ...
        --aligner "bowtie2" \
        --threads ~{cpu} \
        --index ~{sub(genome_index[0], ".1.bt2", "")} # get path to index files without extensions \
      | tee decontamination-log.json
