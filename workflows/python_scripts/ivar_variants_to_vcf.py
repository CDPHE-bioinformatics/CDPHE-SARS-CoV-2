# Credit to Jared Simpson- I made a few changes to ivar_variants_to_vcf.py to add some more ivar output columns
...
def ivar_variants_to_vcf(FileIn,FileOut,passOnly=False,minAF=0):
    ...
    sample_id = os.path.splitext(FileOut)[0].split('_')[0]
    header = ('##fileformat=VCFv4.2\n'
              '##source=iVar\n'
              '##INFO=<ID=DP,Number=1,Type=Integer,Description="Total Depth">\n'
              '##INFO=<ID=AF,Number=1,Type=Float,Description="Frequency of alternate base">\n'
              '##INFO=<ID=GFF_FEATURE,Number=1,Type=String,Description="Associated gff feature">\n'
              '##INFO=<ID=REF_CODON,Number=1,Type=String,Description="Reference codon">\n'
              '##INFO=<ID=REF_AA,Number=1,Type=String,Description="Reference amino acid">\n'
              '##INFO=<ID=ALT_CODON,Number=1,Type=String,Description="Alternate codon based on SNP">\n'
              '##INFO=<ID=ALT_AA,Number=1,Type=String,Description="Alternate amino acid based on SNP">\n'
              '##INFO=<ID=POS_AA,Number=1,Type=String,Description="Alternate amino acid gene position">\n'
    ...
    header += '#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\tFORMAT\t'+sample_id+'\n'
        ...
        for line in f:
            if not re.match("REGION",line):
                ...
                if pass_test == 'TRUE':
                    ...
                    FILTER='FAIL'
                INFO='DP='+line[11]+';AF='+line[10]+';GFF_FEATURE='+line[14]+';REF_CODON='+line[15]+';REF_AA='+line[16]+';ALT_CODON='+line[17]+';ALT_AA='+line[18]+';POS_AA='+line[19][:-2]
